...
def generateCSTJson(builddir, alltypes):
    ...
    # Start the header file.
    outf = open(os.path.join(headerdir, "CSTJsonConverter.h"), "w")
    outf.write(
        """//------------------------------------------------------------------------------
//! @file CSTJsonConverter.h
...
class SLANG_EXPORT CSTJsonConverter {
public:
    explicit CSTJsonConverter(JsonWriter& writer);
...
"""
    ...
    # Start the source file.
    cppf = open(os.path.join(builddir, "CSTJsonConverter.cpp"), "w")
    cppf.write(
        """//------------------------------------------------------------------------------
// CSTJsonConverter.cpp
// Concrete Syntax Tree JSON serialization
//
// SPDX-FileCopyrightText: Michael Popoloski
// SPDX-License-Identifier: MIT
//------------------------------------------------------------------------------
#include "slang/syntax/CSTJsonConverter.h"
...
CSTJsonConverter::CSTJsonConverter(JsonWriter& writer) : writer(writer) {}

void CSTJsonConverter::serialize(const SyntaxTree& tree) {
    writer.startObject();
...
"""
    ...
    """
        ...
        for member in typeinfo.combinedMembers:
            ...
            if isOptional:
                cppf.write(f"        if (node.{memberName}) {{\n")
                cppf.write(f'            writer.writeProperty("{memberName}");\n')

                if memberType == "Token":
                    cppf.write(f"            visitToken(node.{memberName});\n")
                else:
                    cppf.write(f"            node.{memberName}->visit(*this);\n")
                ...
                if memberType == "Token":
                    ...
                    cppf.write(
                        f"""
        if (node.{memberName}.empty()) {{
            writer.writeEmptyArray();
        }} else {{
            writer.startArray();
            // SeparatedSyntaxList stores elements and separators alternately
            for (size_t i = 0; i < node.{memberName}.getChildCount(); i++) {{
                auto child = node.{memberName}.childNode(i);
                if (child) {{
                    child->visit(*this);
                }} else {{
                    auto token = node.{memberName}.childToken(i);
                    if (token) {{
                        visitToken(token);
                    }}
                }}
        ...
        if (node.{memberName}.empty()) {{
            ...
            writer.startArray();
            for (auto token : node.{memberName}) {{
                visitToken(token);
            }}
            writer.endArray();
        }}"""
                    ...
                    # Handle other pointer/reference members
                    cppf.write(f"        node.{memberName}->visit(*this);\n")
    ...
    # Close the class and add the implementation
    cppf.write(
        """};

void CSTJsonConverter::serialize(const SyntaxNode& node) {
