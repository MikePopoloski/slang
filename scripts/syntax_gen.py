...
struct CloneVisitor {
    ...
    template<typename U>
    T* visit(const U& node, BumpAllocator& alloc) {
        if constexpr (std::is_same_v<T, U>) {
            return alloc.emplace<T>(node);
        }
        else if constexpr (is_detected_v<clone_t, T> && std::is_base_of_v<T, U>) {
    ...
    T* visitInvalid(const SyntaxNode&, BumpAllocator&) { return nullptr; }
};
    ...
    for k, v in sorted(alltypes.items()):
        ...
        if v.members or v.final != "":
            ...
            if v.final:
                ...
                cppf.write("    CloneVisitor<{0}> visitor;\n".format(k))
                cppf.write("    return visit(visitor, alloc);\n")
            cppf.write("}\n\n")
