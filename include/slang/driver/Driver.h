...
public:
    ...
    [[nodiscard]] auto setCurrentCommandFile(std::filesystem::path path) {
        ...
        auto savedCommandFile = std::move(currentCommandFile);
        currentCommandFile = std::move(path);
        if (!currentCommandFile.empty())
            commandFileMetadata.try_emplace(currentCommandFile);

        return ScopeGuard([this, savedCommandFile = std::move(savedCommandFile)]() mutable {
