//------------------------------------------------------------------------------
//! @file DFAImpl.h
//! @brief The default built-in data flow analysis pass
//
...
class SLANG_EXPORT DFABase : public AbstractFlowAnalysis<TDerived, TState>,
...
public:
    ...
    DFABase(AnalysisContext& context, const Symbol& symbol, bool reportDiags) :
        AFABase(symbol, context.manager->getOptions(),
                reportDiags ? &context.diagnostics : nullptr),
        DataFlowAnalysis(context, this->getState().assigned), context(context),
...
template<typename TDerived, typename TState>
void DFABase<TDerived, TState>::noteReference(const ValueSymbol& symbol, const Expression& lsp) {
...
class SLANG_EXPORT DefaultDFA : public DFABase<DefaultDFA, DataFlowState> {
public:
    DefaultDFA(AnalysisContext& context, const Symbol& symbol, bool reportDiags) :
        DFABase(context, symbol, reportDiags) {}
};
