...
//}
//
//ProceduralBlock::Kind proceduralBlockKindFromKeyword(Token kindKeyword) {
...
//		DEFAULT_UNREACHABLE;
//		return ProceduralBlock::Initial;
//	}
//}
//
...
//}
//
//SymbolList SemanticModel::getMembers(const SyntaxList<MemberSyntax>& members, const Symbol& parent) {
//	SmallVectorSized<const Symbol*, 8> results;
//	for (const MemberSyntax* member : members) {
//
//	}
//	return results.copy(alloc);
//}
//
//const TypeSymbol& SemanticModel::getType(const DataTypeSyntax& syntax, const Scope& scope) {
//    switch (syntax.kind) {
//        case SyntaxKind::BitType:
//        case SyntaxKind::LogicType:
//        case SyntaxKind::RegType: {
//            // simple integral vector (possibly of just one element)
//            const auto& its = syntax.as<IntegerTypeSyntax>();
...
//    auto returnType = getType(*proto.returnType, scope);
...
//                type = &getTypeSymbol(*portSyntax->dataType, scope);
...
//            coerceInteger(evaluateConstant(range.left), bad),
//            coerceInteger(evaluateConstant(range.right), bad)
//        });
//
//		if (bad)
//			return false;
...
//}
//
//const std::vector<SemanticModel::ParameterInfo>& SemanticModel::getDeclaredParams(const ModuleDeclarationSyntax& syntax) {
...
//        ExpressionBinder binder { *this, scope };
//        const auto& ports = instanceModuleSyntax.header.ports->as<AnsiPortListSyntax>().ports;
//        for (auto port : ports) {
...
//                const auto& ordered = conn->as<OrderedPortConnectionSyntax>();
...
//}
//
//void SemanticModel::getAttributes(SmallVector<const AttributeSymbol*>& results, const SyntaxList<AttributeInstanceSyntax>& attributes) {
//    struct Entry {
...
//			ConstantValue value(SVInt::One, getKnownType(SyntaxKind::BitType));
//			results.append(alloc.emplace<AttributeSymbol>(*attr, value));
//        }
//        else {
//			ConstantValue value = evaluateConstant(attr->value->expr);
//			results.append(alloc.emplace<AttributeSymbol>(*attr, value));
//        }
//    }
//}
...
//	ExpressionBinder binder { *this, scope };
//	auto expr = binder.bindConstantExpression(syntax);
//    if (expr->bad())
//        return nullptr;
//
